  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    if (type === 'question') {
      ...
      const sourceQuestionIndex = parseInt(
        source.droppableId.split('-').pop() || '0'
      );
      const destQuestionIndex = parseInt(
        destination.droppableId.split('-').pop() || '0'
      );

      if (sourceQuestionIndex === destQuestionIndex) {
        const optionsFieldArray = form.getValues(`questions.${sourceQuestionIndex}.options`) || [];
        const newOptions = [...optionsFieldArray];
        const [movedOption] = newOptions.splice(source.index, 1);
        newOptions.splice(destination.index, 0, movedOption);

        form.setValue(`questions.${sourceQuestionIndex}.options`, newOptions);
      }
    }
  };
